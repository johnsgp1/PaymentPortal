...
var Mes = {
    ...
    tokenize: function(cc, exp, cb) {
        ...
        if (!cors)
            Mes.complete({'code': 1, 'gateway_text': 'Unsupported Browser' }, cb);
        // Validate Card num with LUHN
        else if(!Mes.valCc(cc))
            Mes.complete({'code': 2, 'gateway_text': 'Invalid Card Number' }, cb);
        // Validate expiry date
        else if(!Mes.valExpiry(exp))
            Mes.complete({'code': 3, 'gateway_text': 'Invalid Expiry Date' }, cb);
        else {
            // CORS Error Handler
            cors.onerror = function() {
                Mes.complete({'code': 6, 'gateway_text': 'Transmission Error' }, cb);
            };
            // CORS Finish Handler
            cors.onload = function() {
                ...
                if(typeof cors.status !== "undefined" && cors.status !== 200)
                    Mes.complete({'code': 5, 'gateway_text': 'Http code '+cors.status+' recieved' }, cb)
                else {
      ...
      //cors.send("transaction_type=T&profile_id='94100005428200000002'&profile_key='vjwSWbsIuGFyiEcMXRBzeSShajjHQNHl'&card_number="+cc+"&card_exp_date="+exp+"&resp_encoding=json&rctl_card_number_truncated=Y&rctl_resp_hash=Y");
